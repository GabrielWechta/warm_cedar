...
if length(ARGS) == 2
	...
	mat_holder = read_matrix_from_file(pwd() * "/" * mat_file_name)
	gen_b, n = read_right_side_vector_from_file(vec_file_name)
	measurment = do_lu(mat_holder[1], gen_b, mat_holder[2], mat_holder[3])

	x = solve_lu(measurment[1], measurment[2], mat_holder[2], mat_holder[3])
	save_results_to_file(pwd() * "/" * "lu_sol.txt", x, mat_holder[2], true)

	"""LU MAIN PART"""
	mat_holder = read_matrix_from_file(pwd() * "/" * mat_file_name)
	gen_b, n = read_right_side_vector_from_file(vec_file_name)
	measurment = do_lu_with_main_element(mat_holder[1], gen_b, mat_holder[2], mat_holder[3])

	x = solve_lu_with_main_element(measurment[1], measurment[2], measurment[3], mat_holder[2], mat_holder[3])
